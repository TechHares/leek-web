<template>
  <div class="label-generator-management-page">
    ...
    <el-dialog
      :title="dialogStatus === 'create' ? '创建标签生成器' : '编辑标签生成器'"
        ...
        <el-form-item label="参数" prop="params">
          <div class="param-scroll-area">
            <DynamicForm
              ...
              :key="dynamicFormKey"
            />
            <div v-else style="color: #b0b3b8;">请选择标签生成器类型后填写参数</div>
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
...
import { getLabelGenerators, createLabelGenerator, updateLabelGenerator, deleteLabelGenerator, getLabelGeneratorTemplates } from '@/api/label_generator'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      try {
        const { data } = await getLabelGenerators(this.listQuery)
        this.list = data
        this.total = data.length
      } catch (error) {
        this.$message.error('获取标签生成器列表失败')
      }
      this.listLoading = false
    },
    async getTemplates() {
      try {
        const { data } = await getLabelGeneratorTemplates()
        this.templates = data
      } catch (e) {
        this.$message.error('获取标签生成器模板失败')
      }
    },
    handleFilter() {
    ...
    async createData() {
      this.$nextTick(() => {
        this.$refs['dataForm'].validate(async (valid) => {
          if (valid) {
            try {
              await createLabelGenerator(this.temp)
    ...
    async updateData() {
      this.$nextTick(() => {
        this.$refs['dataForm'].validate(async (valid) => {
          if (valid) {
            try {
              await updateLabelGenerator(this.temp.id, this.temp)
              this.dialogFormVisible = false
    ...
    async handleDelete(row) {
      try {
        await this.$confirm('确认删除该标签生成器?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
        await deleteLabelGenerator(row.id)
    ...
    async handleEnableChange(row, val) {
      try {
        await updateLabelGenerator(row.id, { is_enabled: val })
...
.label-generator-management-page {
  padding: 20px;
}
.el-card {
  margin-bottom: 20px;
